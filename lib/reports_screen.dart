...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _loadWeeklyData() async {
    ...
    String? storedData = LocalStorage().getWeeklyData();
    if (storedData == null) {
      Map<String, List<int>> mixedData = {
        for (var habit in selectedHabits)
          habit.title: List.generate(
              7,
                  (_) =>
              Random().nextBool() ? 1 : 0), // Generate a mix of 0s and 1s
      };
      await LocalStorage().setWeeklyData( jsonEncode(mixedData));
      ...
      return DataRow(
        cells: [
          ...
          ...List.generate(7, (index) {
            bool isCompleted = weeklyData[habit.title]?[index] == 1;
            return DataCell(
              Icon(
